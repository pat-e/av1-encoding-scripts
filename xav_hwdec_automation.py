...
# Same as xav_automation.py otherwise. Use this when the GPU decoder is available.
...
def run_xav(xav_input, xav_output, track, preset_override=None, tune_override=None):
    ...
    print(
        ...
        f"-b {buff}  --hwdec  (no -a: audio is this script)"
    )
    print("    - GPU decode (--hwdec): no CPU fallback. If this fails, use xav_automation.py instead.")

    encoder_params = f"--preset {preset} --tune {tune}"
    xav_args = ["xav", "-e", XAV_ENCODER]
    if XAV_HWDEC:
        xav_args.append("--hwdec")
    xav_args += [
        "-p", encoder_params,
        "-w", str(workers),
        "-b", str(buff),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Same as xav_automation.py, plus xav --hwdec (GPU decode, no CPU fallback)."
    )
    parser.add_argument(
        "--no-downmix",
        action="store_true",
